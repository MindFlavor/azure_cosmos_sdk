use authorization_token::{AuthorizationToken, TokenType};

use database::Database;
use collection::Collection;
use document::{DocumentAttributes, IndexingDirective};
...
use request_response::{CreateDatabaseRequest, Document, GetDocumentAdditionalHeaders,
                       GetDocumentResponse, ListCollectionsResponse, ListDatabasesResponse,
                       ListDocumentsResponse, ListDocumentsResponseAdditionalHeaders,
                       ListDocumentsResponseAttributes, ListDocumentsResponseEntities,
                       QueryDocumentResponse, QueryDocumentResponseAdditonalHeaders,
                       QueryResponseMeta, QueryResult};
use azure_core_sdk::COMPLETE_ENCODE_SET;

use ConsistencyLevel;
use list_documents::ListDocumentsOptions;
use get_document::GetDocumentOptions;
use query_document::QueryDocumentOptions;
use azure_core_sdk::incompletevector::ContinuationToken;
use query::Query;
use partition_key::PartitionKey;
    ...
    match status {
        ...
        _ => {
            ...
            Err(AzureError::UnexpectedHTTPResult(
                UnexpectedHTTPResult::new(StatusCode::Ok, status, error_text),
            ))
        }
    }
}
